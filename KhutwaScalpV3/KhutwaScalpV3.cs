...
namespace cAlgo.Robots
{
    ...
    public class KhutwaScalpV3 : Robot
    {
        [Parameter("Demo ONLY (stop on live)", DefaultValue = true)]
        public bool DemoOnly { get; set; }

        [Parameter("Bot Label", DefaultValue = "Khutwa-Scalp-V3")]
        public string BotLabel { get; set; }

        [Parameter("Risk % per Trade", DefaultValue = 0.5, MinValue = 0.01, MaxValue = 2.0)]
        public double RiskPercent { get; set; }

        [Parameter("Max Trades / Day", DefaultValue = 25, MinValue = 1, MaxValue = 60)]
        public int MaxTrades { get; set; }

        [Parameter("Max Daily Loss %", DefaultValue = 2.5, MinValue = 0.5, MaxValue = 10.0)]
        public double MaxDailyLossPercent { get; set; }

        [Parameter("Stop after N Losses", DefaultValue = 3, MinValue = 1, MaxValue = 10)]
        public int MaxLosingStreak { get; set; }

        [Parameter("Cooldown Seconds", DefaultValue = 60, MinValue = 0, MaxValue = 1800)]
        ...
        [Parameter("Min Volume Ratio", DefaultValue = 0.95, MinValue = 0.25)]
        public double MinVolumeRatio { get; set; }

        [Parameter("Min Score", DefaultValue = 3.25, MinValue = 2.0, MaxValue = 5.0)]
        public double MinScore { get; set; }

        [Parameter("Min ATR (pips)", DefaultValue = 0.7, MinValue = 0.1)]
        public double MinAtrPips { get; set; }

        [Parameter("Max ATR (pips)", DefaultValue = 7.0, MinValue = 0.2)]
        public double MaxAtrPips { get; set; }

        [Parameter("Stop ATR Multiplier", DefaultValue = 1.3, MinValue = 0.2)]
        ...
        [Parameter("Maximum Stop (pips)", DefaultValue = 10.0, MinValue = 1.0)]
        public double MaxStopPips { get; set; }

        [Parameter("Reward / Risk", DefaultValue = 1.5, MinValue = 0.5)]
        public double RewardRisk { get; set; }

        [Parameter("Max Spread (pips)", DefaultValue = 0.8, MinValue = 0.1)]
        public double MaxSpreadPips { get; set; }

        // Reserve estimate, NOT a measurement of your broker's commission.
        [Parameter("Extra Round Trip Cost (pips)", DefaultValue = 0.05, MinValue = 0.0)]
        public double ExtraCostPips { get; set; }

        [Parameter("Max Estimated Cost / SL", DefaultValue = 0.18, MinValue = 0.01, MaxValue = 0.75)]
        public double MaxCostToStop { get; set; }

        [Parameter("Break Even At (R)", DefaultValue = 0.9, MinValue = 0.2)]
        public double BreakEvenAtR { get; set; }

        [Parameter("Break Even Lock (R)", DefaultValue = 0.05, MinValue = 0)]
        public double BreakEvenLockR { get; set; }

        [Parameter("Time Exit (minutes)", DefaultValue = 10, MinValue = 1)]
        ...
        protected override void OnStart()
        {
            ...
            {
                Print("SAFETY: V3 is DEMO ONLY. Live account detected; refusing to trade.");
                Stop();
                return;
            }
            ...
            {
                Print("SETUP ERROR: attach V3 to EURUSD M1; current timeframe is {0}.", TimeFrame);
                Stop();
                return;
            }

            if (FastPeriod >= SlowPeriod || M5FastPeriod >= M5SlowPeriod ||
                MinStopPips > MaxStopPips || MinAtrPips >= MaxAtrPips)
            {
            ...
            Print("V3 RUNNING | DEMO={0} | Symbol={1} M1 entries + M5 CLOSED-BAR trend | risk={2:F2}% | trades/day={3}",
                DemoOnly, SymbolName, RiskPercent, MaxTrades);
            Print("Fee guard: live spread + extra COST ESTIMATE {0:F2}p. Check broker-reported fees in CLOSED logs.", ExtraCostPips);
        ...
        protected override void OnBarClosed()
        {
            ...
            {
                Skip("Managing open V3 position; no overlapping positions.");
                return;
            }

            // Do not run V2 and V3 simultaneously on the same EURUSD symbol and account.
            bool otherKhutwa = Positions.Any(p => p.SymbolName == SymbolName &&
                p.Label != null && p.Label.StartsWith("Khutwa-Scalp-") && p.Label != BotLabel);
            if (otherKhutwa) { Skip("Another Khutwa bot has an active EURUSD position."); return; }
            ...
            if (spread <= 0) { Skip("Invalid/zero spread quote."); return; }
            if (spread > MaxSpreadPips)
            {
                Skip(string.Format("Spread too wide: {0:F2} > {1:F2}p.", spread, MaxSpreadPips));
                return;
            }
            ...
            {
                Skip(string.Format("Volatility outside range; ATR={0:F2}p allowed={1:F1}-{2:F1}.",
                    atrPips, MinAtrPips, MaxAtrPips));
                return;
            }

            double stop = Math.Max(MinStopPips, Math.Max(atrPips * StopAtrMultiplier, 3.0 * spread));
            if (stop > MaxStopPips) { Skip("Required stop exceeds max allowed stop."); return; }

            double costRatio = (spread + ExtraCostPips) / stop;
            if (costRatio > MaxCostToStop)
            {
                Skip(string.Format("Estimated cost/stop {0:F3} > {1:F3}, spread={2:F2}p SL={3:F2}p.",
                    costRatio, MaxCostToStop, spread, stop));
                return;
            }
        ...
        private void OpenTrade(TradeType type, double score, double stopPips, double initialSpread, string trend)
        {
            if (RiskBlocked()) return;
            double currentSpread = (Symbol.Ask - Symbol.Bid) / Symbol.PipSize;
            if (currentSpread <= 0 || currentSpread > MaxSpreadPips ||
                (currentSpread + ExtraCostPips) / stopPips > MaxCostToStop)
            {
                Skip("Execution check: quote cost moved outside permitted range.");
                return;
            }

            // Conservative size reserve: costs and stop distance share the risk budget.
            double effectiveRisk = RiskPercent * stopPips / (stopPips + currentSpread + ExtraCostPips);
            double units = Symbol.VolumeForProportionalRisk(
                ProportionalAmountType.Equity, effectiveRisk, stopPips, RoundingMode.Down);
            units = Symbol.NormalizeVolumeInUnits(units, RoundingMode.Down);
            if (units < Symbol.VolumeInUnitsMin)
            {
                Skip(string.Format("Minimum broker volume would exceed risk budget: calc={0}, minimum={1}.",
            ...
            var result = ExecuteMarketOrder(type, SymbolName, units, BotLabel,
                stopPips, stopPips * RewardRisk);
            if (!result.IsSuccessful || result.Position == null)
            {
                Print("ORDER_FAILED side={0} error={1}", type, result.Error);
                return;
            }
            ...
            {
                Spread = currentSpread, Quote = quote, InitialStop = stopPips,
                EstimatedExtra = ExtraCostPips, Score = score, Trend = trend
            };
            Print("OPEN id={0} type={1} score={2:F2} trend={3} volume={4} quoteSpread={5:F2}p extraCostEstimate={6:F2}p SL={7:F2}p TP={8:F2}p adverseSlippage={9:F2}p",
                p.Id, type, score, trend, units, currentSpread, ExtraCostPips,
                stopPips, stopPips * RewardRisk, adverseSlippage);
        }
        ...
        private void OnPositionClosed(PositionClosedEventArgs args)
        {
            ...
            if (_audit.TryGetValue(p.Id, out audit))
            {
                Print("CLOSED id={0} reason={1} durationSec={2:F0} entrySpread={3:F2}p extraCostEST={4:F2}p initialStop={5:F2}p gross={6:F2} commissions={7:F2} swap={8:F2} net={9:F2} realisedPips={10:F1}",
        ...
        {
            Positions.Closed -= OnPositionClosed;
            Print("V3 stopped. Never infer strategy profitability from a short session.");
        }
    }
}
